...
- 弹性任务：参与自动重排，系统可以根据优先级、截止时间和冲突情况移动它。
- 固定任务：锁定在当前时间段，不参与本地规则重排，也会作为 AI 重排接口里的硬约束保留。
...
固定任务会被当作 locked/hard constraint。即使调用 `/api/scheduling/replan` 或 DeepSeek 策略建议，固定任务也不会被移动、压缩、删除或重命名；只有弹性任务会参与重排。

拖动任务块时允许短暂重合。系统会把重合识别为冲突输入，再按固定任务优先、高优先级优先、截止时间优先、低优先级弹性任务先后移的策略自动优化。
...
- 固定导入：适合课程表、会议表、考试、线下约定，写入后不参与重排。
- 弹性导入：适合学习计划、待办安排、可调整时间块，写入后参与动态重排。
